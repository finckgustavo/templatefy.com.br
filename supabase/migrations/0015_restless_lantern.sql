/*
  # Fix user limits migration

  1. Changes
    - Fix user limits table with proper email access
    - Add RLS policies for proper access control
    - Insert default limits for existing users
    - Set up admin user correctly
...
SELECT 
  ...
  10,
  CASE 
    WHEN u.raw_user_meta_data->>'email' = 'finckgustavo@gmail.com' THEN true 
    ELSE false 
  END
FROM auth.users u
...
SET 
  ...
  END,
  is_admin = CASE 
    WHEN auth.users.raw_user_meta_data->>'email' = 'finckgustavo@gmail.com' THEN true 
